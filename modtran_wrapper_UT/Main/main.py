...
class BuildTape5(object):
    ...
    def assemble_tape5(self, mat):
        ...
        self.card1 = self.modtran_functions.build_card1(self.modtran_functions.find_in_array(mat, 'MODTRN'), self.modtran_functions.find_in_array(mat, 'SPEED'),
                                                        self.modtran_functions.find_in_array(mat, 'BINARY'),self.modtran_functions.find_in_array(mat, 'LYMOLC'),
    ...
    def execute_modtran(self, parameter, beginning, end, step, list_values):
        ...
        SPEED = self.stan_parm.SPEED
        BINARY = self.stan_parm.BINARY
        ...
        for key in parameter_mat:
            if key[1] == parameter:
                '''checks if the parameter given as function input matches a parameter in parameter_mat'''
                if list_values == []:
                    '''only uses beginning/end/step if there is no parameter given in parameter_mat'''
                    key[0] = self.modtran_functions.adjust_length(str(beginning), key[2])
                    self.card5 = '    1!card 5.'
                    self.write_tape5(mat = parameter_mat)
                    
                    i = beginning
                    '''here the values for the different steps are calculated and then a tape5 added to the .tp5 file'''
                    while i < end-step:
                        i = np.add(i, step)
                    ...
                    i2 = 0
                    while i2 < len(list_values)-1:
                        '''takes all values except for the last in list_values and writes a tape5 with each value to the .tp5 file'''
